...
def save_locs(path,locs,info,mode=None):
  ...
  https://webmail.cup.uni-muenchen.de/webmail/rcube/?_task=mail&_action=get&_mbox=INBOX&_uid=3762&_token=sN6lTf9kSQdofyokV1Rupu9GmL82v7a2&_part=3  path : str
...
def _save_picks(locs,pick_diameter,path):
    '''
    Extract x and y positions out of the locs file and save them stored as a dictionary in .yaml file.
    ...
    locs: pandas.DataFrame
        locs (as in picasso) but converted to pd.DataFrame format 
    pick_diameter: float
        Diameter the circle around the pick positions
    path : str
        Path in which picks are stored
    '''
    centers = locs[['x', 'y']].values.tolist()
