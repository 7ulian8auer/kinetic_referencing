        ...
        while data_set.iloc[k].y < data_set.iloc[i].y+d_max:
            ...
            if d_min < dist < d_max:
                ...
                if math.isnan(data_set.iloc[i,p]):
                    tau_diff = abs(data_set.iloc[i,6]-data_set.iloc[k,6])
...
def picks_save(data_set_assigned,picks,path):
    ...
    pick_diameter = 1
    lbfcs.io._save_picks(pic_wrap._coordinate_convert(single_picks, (0,0), oversampling),
                         pick_diameter,
    ...
    pick_diameter = 2
    lbfcs.io._save_picks(pic_wrap._coordinate_convert(pair_picks, (0,0), oversampling),
                         pick_diameter,
...
def outlier_plot(data_sec_initial, label, save_path):
    data_names=['taud_diff','n_locs_diff','mod_n_locs','ref_n_locs','n_events_diff','mod_n_events','ref_n_events']
    ...
    istrue = istrues[0] + istrues[1] + istrues[2] + istrues[3] + istrues[4] + istrues[5] + istrues[6] + istrues[7] + istrues[8] + istrues[9] + istrues[10] + istrues[11] + istrues[12] + istrues[13] 
